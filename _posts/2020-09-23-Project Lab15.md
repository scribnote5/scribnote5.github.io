...
## 프로젝트 실행 및 결과
- 벡엔드에서 title(@NotBlank)의 유효성 검사를 실패하면 다음 이미지와 같이 응답 메시지를 확인할 수 있다.

![image](/assets/images/2020-09-23-Project Lab15/image1.png)


- title(@NotBlank)의 유효성 검사를 실패하면, 해당 메시지를 json 자료형으로 형변환 하고 parsing하여 사용자에게 alert 경고창으로 유효성 검사 실패 결과를 알려준다.

![image](/assets/images/2020-09-23-Project Lab15/image2.png)

- 20 MB 보다 큰 파일을 업로드 할 때 유효성 검사에 실패하여 발생하는 에러 메시지다.
