...
excerpt: "리눅스 커널에 Xenmoai 패치를 적용한 다음 빌드하여 ARM Sabre Lite의 리눅스 커널  변경하기"
...
- 리눅스 커널에 Xenmoai 패치를 적용한 다음 빌드하여 ARM Sabre Lite의 리눅스 커널 변경하는 과정을 소개한다.
...
- Xenomai는 RTOS API를 제공하며, 이를 사용하여 실시간 태스크를 생성할 수 있다.
- VxWorks, pSOS 그리고 Alchemy라는 자체 RTOS API를 지원하며, 에뮬레이터를 사용하여 다른 RTOS의 API를 적은 오버헤드로 모방할 수 있다. 
...
- 커널이 실시간성을 가지기 위해서는 PREEMPT-RT 패치가 필요하다. 즉 실시간성은 PREEMPT-RT 패치에 의존적이다.
...
\# 커널 컴파일 설정으로 특별히 설정할 항목은 없으며 사용자 필요에 따라 수정 <br>
$  make menuconfig

\# 해당 디렉토리에 커널 모듈 설치 <br>
$ mkdir modules
...
\# 링크 파일 생성 <br>
$ ln -s boot/initrd.img-4.14.85 initrd.img<br>
...
$ sudo tar --numeric-owner -xf /home/linux-4.14.85.tar.gz -C /<br>
$ sudo update-initramfs -c -k4.14.85<br>

$ rm /home/linux-4.14.85.tar.gz<br>

$ sudo sync && sudo reboot



## 재부팅 후 Xenomai 적용 메시지 확인 
$ dmesg | grep Xenomai
