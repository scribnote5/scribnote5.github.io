...
- 소프트웨어 신뢰성시험은 정적시험(코딩규칙 검증, 취약점 점검, 소스코드 메트릭 점검)과 동적시험(코드실행률 점검)으로 구분하여 수행한다.



## 정적시험
- 소프트웨어를 실행하지 않은 상태에서 잠재적인 결함을 검출하는 시험을 말하며, 코딩 규칙(Coding Rule) 검증 시험, 취약점 점검 시험 및 소스코드 메트릭(Code Metrics) 점검을 의미한다.
...
- 적용 대상 언어는 C, C++, C#, JAVA다.

#### 대상 언어별 적용되는 표준
- C: MISRA C: 2012
- C++: MISRA C++: 2008, JSF++(Koint Strike Figher Air Vehicle C++) 
=> 일반적으로 MISRA C++: 2008 규칙으로 검증한다.
- Java: Code conventions for the Java Programming Language(Oracle)
- C#: C# Coding conventions 또는 .NET Framework Design Guideline(Microsoft)
...
<https://scribnote5.github.io/categories/#misra-c>


#### C언어 보안 취약점 방지를 위한 시큐어 코딩 표준
- 시큐어 코딩 표준에는 CWE, CERT C, IS/IOEC TS 17961:2013, C Secure, MISRA C가 존재한다.
...
- 소스코드 메트릭에 대한 자세한 내용은 다음 글에서 확인할 수 있다. <br>
<https://scribnote5.github.io/code%20metrics/static%20test/Code-Metrics1/>
...
- MISRA C/C++ 또는 CERT C를 준수한다면, CWE에서 선정한 취약점도 보완할 수 있다. 
- 일반적인 무기체계 소프트웨어에서는 보안성 시험을 수행하지 않는다. 전장관리시스템, 그룹웨어 같은 극히 제한된 시스템에서 보안성 시험을 수행한다. 보안성 시험은 행정안전부의 '소프트웨어 개발보안 가이드'를 준수해야 한다.
- CWE에 대한 자세한 내용은 다음 글에서 확인할 수 있다. <br>
<https://scribnote5.github.io/cwe/static%20test/CWE658-1/>
...
- 동적 시험에 대한 자세한 내용은 다음 글에서 확인할 수 있다. <br>
<https://scribnote5.github.io/code%20metrics/static%20test/dynamic test1/>
