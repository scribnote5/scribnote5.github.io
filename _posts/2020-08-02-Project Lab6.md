...
- 많은 블로그에서 고민한 결과대로, 본 프로젝트에서는 Service 계층까지 DTO를 사용할 것이다. 또한, DTO가 굳이 필요 없는 부분에서는 사용하지 않는 방식으로 DTO를 유연하게 사용할 것이다.
...
- Pageable(Paging을 담당하고 NoticeBoard 리스트) 객체를 반환하는 소스 코드는 하단 출처를 참고하였다. 
