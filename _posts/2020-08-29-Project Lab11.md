...
excerpt: "Spring Data JPA의 Audit을 사용하여 Table에서 공통으로 사용하는 데이터(생성자, 수정자, 생성일, 수정일)를 저장하는 방법을 소개한다."
...
---
- Spring Data JPA의 Audit을 사용하여 Table에서 공통으로 사용하는 데이터(생성자, 수정자, 생성일, 수정일)를 저장하는 방법을 소개한다.
...
## JPA Audit
- 현재 프로젝트 내 Domain은 CommonAudit(일반적인 데이터) 또는 AttachedFileAudit(첨부 파일 데이터)를 상속받는다. 이를 통해서 생성일자, 수정일자, 생성자, 수정자와 같이 모든 Table에서 공통적으로 사용하고 있는 컬럼과 1:1 매칭되도록 하였다. 
...
- 하지만 생성자 및 수정자를 자동으로 입력하는 @Createdyby, @LastModifiedBy 애노테이션의 경우, 게시글이 Create 및 Update 되는 모듈은 module-app-web이 아니라 module-app-api 모듈이므로 해당 애노테이션을 적용할 수 없다. 즉 사용자는 module-app-web 모듈에서 Spring Security로 로그인하였기에 사용자 데이터를 유지하고 있지만, module-app-api 모듈에서 Spring Security로 로그인되어 있지 않기 때문에 사용자 데이터가 존재하지 않는다. @CreatedBy, @LastModifiedBy 애노태이션을 대신하여, form 페이지의 input 태그를 사용하여 생성자 및 수정자 정보를 전송하도록 구현하였다.
- 만약 Spring Security를 적용한 단일 프로젝트에서 @Createdyby, @LastModifiedBy을 적용한다면 AuditorAware 인터페이스를 구현받으면 된다. 자세한 내용은 다음 출처를 참고하면 된다. 
...
## 비인증 사용자 접속 차단
- form 페이지의 input 태그를 사용하여 생성자 및 수정자 정보를 전송하도록 구현하기 위한 전제 조건은, form 페이지에는 인증 사용자만 접근해야 한다.
...
- .authenticationEntryPoint(new CustomAuthenticationEntryPoint());: 비인증 사용자가 접근하는 경우 발생하는 에러를 CustomAuthenticationEntryPoint 핸들러가 처리한다.
...
<br>
- 비인증 사용자가 인증된 사용자(권한을 요구하는) 페이지에 접근하는 경우 발생하는 예외를 처리한다. Http 상태를 401 에러(UnAuthorized)로 지정하고 비인증 사용자 접근 오류 페이지로 forward한다.
...
<br>
- 첨부 파일의 수정일은 의미 없는 데이터로서, @CreatedDate 애노테이션만 선언하였다.
...
<br>
- DTO <-> Domain 변환에 사용되는 Builder 패턴에 생성자 및 수정자 정보를 저장하는 setCreatedBy(createdBy));, setLastModifiedBy
...
## Service
- 게시글의 경우 DTO에 데이터가 저장되어 전달되지만, 첨부 파일의 경우 데이터를 객체에 저장하여 전달하지 않기 때문에 생성자 데이터를 부가적으로 전달 받는다.
...
## View 
- 생성자 및 수정자 데이터는 input hidden type에 저장되어 Controller로 전달된다.
- authentication 객체는 Spring Security에서 인증한 사용자의 데이터가 저장된다.
...
## 프로젝트 실행 및 결과
- 다음과 이미지와 게시글을 새로 등록하면 생성자, 생성일, 수정일 정보가 등록되어 표시되는 것을 확인할 수 있다.
