...
- 하지만 생성자와 수정자를 자동으로 입력하는 @Createdyby, @LastModifiedBy 애노테이션의 경우, 게시글이 Create 및 Update 되는 모듈은 module-app-web이 아니라 module-app-api 모듈이므로 해당 애노테이션을 적용할 수 없다. 즉 사용자는 module-app-web 모듈에서 Spring Security로 로그인하였기에 사용자 데이터를 유지하고 있지만, module-app-api 모듈에서 Spring Security로 로그인되어 있지 않기 때문에 사용자 데이터가 존재하지 않는다. @CreatedBy, @LastModifiedBy 애노태이션을 대신하여, form 페이지의 input 태그를 사용하여 생성자와 수정자 정보를 전송하도록 구현하였다.
...
## 비인증 사용자 접속 차단
- form 페이지의 input 태그를 사용하여 생성자와 수정자 정보를 전송하도록 구현하기 위한 전제 조건은, form 페이지에는 인증 사용자만 접근해야 한다.
...
<br>
- DTO <-> Domain 변환에 사용되는 Builder 패턴에 생성자와 수정자 정보를 저장하는 setCreatedBy(createdBy));, setLastModifiedBy
