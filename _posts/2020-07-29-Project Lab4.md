...
## 프로젝트 소개
- <span style="color:red; font-weight: bold">본 게시글에서는 Spring Boot을 사용한 기본적인 CRUD 게시판 소스 코드를 제공한다.(github commit 참고)</span>
- 프로젝트의 기본 프레임(CRUD, Spring Security 등)은 '처음 배우는 스프링 부트 2' 도서를 참고 하였으며, '이런 부분에서는 어떻게 코딩하면 가독성과 성능을 향상시킬 수 있을까?'라는 생각을 가지고 프로젝트를 리펙토링 하였다. 
- 게시글에서 Spring Boot에 대한 기본 지식과 프로젝트의 세부 메커니즘 설명은 생략하고 소스 코드는 github commit으로 대체한다. 만약 Spring Boot에 대한 기본 지식이 부족하고 프로젝트의 전체 흐름이 잘 이해 되지 않는다면 도서와 하단 웹 페이지들을 참고하는 것을 추천한다. 
...
- yml 파일은 설정 및 기타 정적 값을 키 값 형식으로 관리하므로, properties 파일보다 가독성이 좋다. 
- 따라서 기존 application.properties 파일을 application.yml 파일로 변경하였다. 



## RESTful
- 도서에서는 RESTful의 제약 조건을 지키기 위해서 두 개의 서버로 분리하였다. 
- View를 담당하는 서버와 REST API를 제공하는 서버 총 2개가 동시에 실행된다. 
...
- 기존 소스 코드는 getOne으로 레퍼런스를 가져왔지만, 서로 다른 트랜잭션에서 사용하였기에 위와 같은 에러가 발생한다. 
...
- ajax 통신을 위한 JSON 객체를 생성할 때 필요한 반복되는 변수 초기화 코드는 작성하기 번거롭고 비효율적이기에, 이런 문제점을 해결하기 위해서 form 태그를 사용하는 경우 input tag의 value를 JSON 데이터로 생성하는 Javascript serializeObject 플러그인을 사용하였다.
- form 태그를와 같이 Javascript serializeObject 플러그인을 사용하면, 하단 예제와 같이  반복되는 변수 초기화 소스 코드를 제거할 수 있다. 
- 참고로 일반적으로 html에서 서버로 데이터를 form 태그를 사용하지만 이는 선택 사항이며, 도서에서는 form 태그를 사용하지 않는다. 
...
- HTML id와 name의 명명 규칙은 각 회사마다 다르게 정의하였기에 정답이 없다. 대표적으로 NHN은 snake case를 사용하고 다음에서는 camel case를 사용한다. 
-  Javascript serializeObject 플러그인을 사용하기 위해서는 Java의 멤버 필드명과 Javascript의 변수명이 같아야 하므로, HTML id와 name 속성 명명규칙으로 camel case(createdDate)을 적용하였다.
- 참고로 도서에서는 HTML id가 snake case(created_date)를 사용하고 있다.



## 'list.html' 게시글 생성일자로 내림차순 
- 게시글 idx(PK) 순서대로 내림 차순으로 정렬되도록 변경하였다.
- 참고로 도서에서는 가장 최근에 작성한 게시글이 가장 첫번 째로 게시글로 등록된다.
...
## 프로젝트 실행 및 결과
- Run -> Edit Configureations... -> module-app-api과 module-app-web 모듈의 Profile을 'local'로 변경 후 두 모듈을 동시에 실행한다.
