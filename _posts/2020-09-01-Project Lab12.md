...
- 다음 출처를 참고하면 JPA에서 대량의 데이터 삭제할 때 성능 문제가 발생한다. 해당 문제를 해결하기 위해서 JPA 대신 QueryDsl로 쿼리를 작성하여 게시글과 연관된 모든 댓글 삭제 기능을 구현하였다.(물론 대량의 데이터일 가능성도 없고, 많이 사용되는 기능이 아니지만...)
